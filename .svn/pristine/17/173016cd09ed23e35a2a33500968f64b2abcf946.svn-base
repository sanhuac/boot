...
public class CarController extends BaseController{
    ...
    @ApiOperation(value="完善车辆信息", notes="完善车辆信息")
    @RequestMapping(value = "secondAddCar",method = RequestMethod.POST)
    @LoginNotRequired
    public ResponseEntity<JsonResult> secondAddCar(CarSecondModel model){
        log.info("完善车辆信息参数:"+model);
        JsonResult result = new JsonResult();
        try {
            Map<String,Object> resultMap = new HashMap<String,Object>();
            carInfoService.secondSave(model);
            resultMap.put("result","完善成功");
            result =dealSuccess (resultMap);
        } catch (Exception e) {
            result = dealException(e);
        }
