    ...
    public Map<String, Object> merCarList(Integer mer_id, CarQueryObject qo) throws UnifiedException {
        ...
        if (count > 0) {
            ...
            for (CarInfo carInfo : list) {
                carInfo.setIndex_image(Constant.getInstance().PIC_URL_PREFIX + carInfo.getIndex_image());
            }
    ...
    public Map<String, Object> productEdit(Integer mer_id, Integer car_id) throws UnifiedException {
        ...
        if (ValidatorUtils.isNotNull(car_id)) {
            ...
            for (CarImage carImage : imageList) {
                carImage.setImage_url(Constant.getInstance().PIC_URL_PREFIX + carImage.getImage_url());
            }
            Map<String, Object> map = new HashMap<>();
            //查询实拍
            if (ValidatorUtils.isNotNull(carInfo.getCar_film_id())) {
                CarFilm carFilm = productMapper.selectCarFilm(carInfo.getCar_film_id());
                carFilm.setFilm_image(Constant.getInstance().PIC_URL_PREFIX + carFilm.getFilm_image());
